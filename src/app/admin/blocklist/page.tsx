...
import {
  getFirestore, doc, setDoc, deleteDoc, getDocs, collection, updateDoc, serverTimestamp,
...
export default function BlocklistPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setCountries(cs.docs.map(d => ({ id: d.id, ...(d.data() as any) })).sort((a,b)=>a.id.localeCompare(b.id)));
      setIps(is.docs.map(d => ({ id: d.id, ...(d.data() as any) })).sort((a,b)=>a.id.localeCompare(b.id)));
    };
    load();
  }, [db]);
  ...
  const addCountry = async () => {
    ...
    await setDoc(doc(db, 'block_countries', id), { enabled: true, note: note || '', updatedAt: serverTimestamp() });
    setCountries(prev => [{ id, enabled: true, note }, ...prev.filter(c => c.id !== id)]);
    setInputCountry(''); setNote('');
  };

  const addIP = async () => {
    const id = inputIP.trim();
    if (!id) return alert('IPを入力してください（IPv4/IPv6 可）');
    await setDoc(doc(db, 'block_ips', id), { enabled: true, note: note || '', updatedAt: serverTimestamp() });
        ...
        {tab === 'country' ? (
          <>
            <div>
              ...
              <input value={inputCountry} onChange={e=>setInputCountry(e.target.value)}
                placeholder="JP / US など" className="border rounded px-2 py-1 w-40" />
            </div>
            <div className="flex-1">
              <label className="block text-sm mb-1">メモ（任意）</label>
              <input value={note} onChange={e=>setNote(e.target.value)}
                placeholder="理由など" className="border rounded px-2 py-1 w-full" />
            </div>
            <button onClick={addCountry}
              className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">追加</button>
          </>
        ) : (
          <>
            <div className="flex-1">
              ...
              <input value={inputIP} onChange={e=>setInputIP(e.target.value)}
                placeholder="1.2.3.4 または IPv6" className="border rounded px-2 py-1 w-full" />
            </div>
            <div className="flex-1">
              <label className="block text-sm mb-1">メモ（任意）</label>
              <input value={note} onChange={e=>setNote(e.target.value)}
              ...
              <tr key={c.id} className="odd:bg-white even:bg-gray-50">
                ...
                <td className="p-2 border">
                  <label className="inline-flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs border ${c.enabled?'bg-green-100 text-green-800 border-green-300':'bg-gray-100 text-gray-700 border-gray-300'}`}>
                      {c.enabled ? 'ON' : 'OFF'}
                    </span>
                  </label>
                </td>
                ...
                <td className="p-2 border">
                  <button onClick={()=>removeCountry(c.id)}
                    className="px-2 py-1 rounded border hover:bg-red-50">削除</button>
                </td>
              </tr>
            ))}
            {countries.length===0 && <tr><td className="p-4 text-center text-gray-500" colSpan={4}>なし</td></tr>}
          </tbody>
        </table>
      ) : (
        <table className="mt-4 w-full border text-sm">
              ...
              <tr key={p.id} className="odd:bg-white even:bg-gray-50">
                ...
                <td className="p-2 border">
                  <label className="inline-flex items-center gap-2">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs border ${p.enabled?'bg-green-100 text-green-800 border-green-300':'bg-gray-100 text-gray-700 border-gray-300'}`}>
                      {p.enabled ? 'ON' : 'OFF'}
                    </span>
                  </label>
                </td>
                ...
                <td className="p-2 border">
                  <button onClick={()=>removeIP(p.id)}
                    className="px-2 py-1 rounded border hover:bg-red-50">削除</button>
                </td>
              </tr>
            ))}
            {ips.length===0 && <tr><td className="p-4 text-center text-gray-500" colSpan={4}>なし</td></tr>}
          </tbody>
        </table>
      )}
    </div>
